...
{
public:
  /// @param freq_hz  Your loop rate in Hz (e.g. 200)
  /// @param beta_gain  Tuning parameter (0.0–1.0), default 0.1
  ...
  imu_madgwick(float freq_hz, float beta_gain = 0.1f)
  {
    sampleFreq = freq_hz;
    beta = beta_gain;
  ...
  /// Call every timestep with raw sensor data:
  /// gyro in rad/s, accel in g’s (i.e. 1.0 = 9.81 m/s²)
  void update(float gx, float gy, float gz,
              float ax, float ay, float az)
  {
    // Madgwick’s IMU-only update (no mag):
    MadgwickAHRSupdateIMU(gx, gy, gz, ax, ay, az);
  }

  /// Returns the fused quaternion as (w, x, y, z)
  std::array<float, 4> get_quaternion() const
  {
    return {q0, q1, q2, q3};
  }
  ...
  {
    // Construct an Eigen quaternion (w, x, y, z)
    float w = q0;
    float x = q1;
    float y = q2;
    float z = q3;
    Eigen::Quaternionf quat(w, x, y, z);
    // Convert to a rotation matrix and extract XYZ (roll, pitch, yaw)
